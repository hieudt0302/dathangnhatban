<?php
include_once ('app/Libraries/com/alibaba/openapi/client/serialize/SerializerProvider.class.php');
include_once ('app/Libraries/com/alibaba/openapi/client/util/SignatureUtil.class.php');
class SyncAPIClient {
	var $clientPolicy;
	...
	public function send(APIRequest $request, $resultDefiniation, RequestPolicy $requestPolicy) {
		...
		if ($data) {
			...
			if ($status >= 400 && $status <= 599) {
				...
				//var_dump($resultDefiniation); die();
			}
		} else {
			$status = curl_getinfo ( $ch, CURLINFO_HTTP_CODE );
			curl_close ( $ch );
